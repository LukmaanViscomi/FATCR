        ...
        try:
            ...
            for obj in items:
                ...
                t0 = _coerce_time_seconds(
                    obj.get("t_start") or obj.get("start") or obj.get("start_time") or obj.get("start_sec") or obj.get("approx_start") or
                    (obj.get("provenance") or {}).get("start") or (obj.get("provenance") or {}).get("start_sec") or (obj.get("provenance") or {}).get("approx_start")
                )
                t1 = _coerce_time_seconds(
                    obj.get("t_end") or obj.get("end") or obj.get("end_time") or obj.get("end_sec") or obj.get("approx_end") or
                    (obj.get("provenance") or {}).get("end") or (obj.get("provenance") or {}).get("end_sec") or (obj.get("provenance") or {}).get("approx_end")
                )
...
def render_claim_card(idx: int, rec: Dict[str, Any]) -> None:
    ...
    if not time_cache:
        # If the app started when processed artefacts were missing, the cache may be empty.
        # Rebuild on-demand so UI timestamps appear once CLAIMS/UTTERANCES artefacts are restored.
        try:
